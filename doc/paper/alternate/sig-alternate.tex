...
software development and more recently have become widely adopted as a result of their
integration with social media environments (GitHub, BitBucket).
...
for RTE systems. However, they essentially differ in that local changes are not merged
immediately (synchronously) into the shared document (as opposed to edit prograpagation
within the RTE framework); instead they are ``commited'' at a later stage (asynchronously)
when the user decides to. By separating edit and merge logic real-time responsiveness is
sacrified but in exchange two crucial features are gained: (i) the concept of a
\textit{working copy}, (ii) changes are meaningful in the sense that succesful merges are
to be considered as intended \textit{versions} of the document. We now turn to these aspects
in more depth.

A \textit{working copy} is a local copy of the shared document that allows the user owning
the copy to start modifying the document despite parallel work by other users
\cite{Collins-Sussman}.
When a user wants to update the central data to reflect the changes in her working copy
...
are pushed, (ii) or there have been incompatible local changes with the shared copy before
this is pulled.
VCS provides a merging mechanism to solve these potential version divergences known as
three-way merging involving all three document versions (conflicting copies and their
shared ancestor) \cite{Altmanninger2009}. 

A useful aspect of working copies is that they allow for more complex and flexible workflows
than the previously depicted of a single working copy. In particular, users may create
multiple working copies addressing disparate aspects of the document and delay merging
to a later stage.
This way, the collaboration process can be seen as a tree of branches (working copies
representing parallel versions of the shared data) that are being splitted and merged with
the root (the shared data).

\textit{Versioning}
\textemdash this is in opposition to RTE document updates that are triggered automatically
