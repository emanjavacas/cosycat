...
\title{
  Enabling Annotation of Historical Corpora in an Asynchronous Collaborative Environment\titlenote{
    The research reported on in this paper is part of the project \textit{Mind-Bending Grammars},
    funded by the ERC Horizon 2020
    (Project ID 639008; \texttt{http://www.uantwerpen.be/mind-bending-grammars/)}.
  }  
}
%% \subtitle{
...
\begin{abstract}
  ...
  needs for synchronizing interdependent annotation metadata by different researchers.
  ...
  is arguably sub-optimal for research purposes.
  In the present paper we discuss potential problems related to synchronizing
  annotations in large-scale projects, as well as the potential benefits
  that can be derived from a dedicated approach to annotation data management.
  As a proof of concept, we finally showcase our current solution in form
  of a collaborative asynchronous application in the context of a Historical Linguistics research
...
\keywords{Corpus Annotation, Asynchronous Collaboration, Groupware}
...
%% Finally, adopting the asynchronous approach towards collaborative research, we also intend to
