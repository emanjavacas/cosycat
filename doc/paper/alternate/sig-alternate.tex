...
\begin{abstract}
  ...
  While well-established software solutions already exist for the corpus data retrieval aspect,
  less attention has been paid to the annotation process in terms of both software
  facilities and best practices, especially in the context of multi-user .
...
not support proper collaborative editing of documents\footnote{
  ...
  of the synchronization functionality to avoid the creation of conflicting copies \textemdash
  see ~https://www.dropbox.com/en/help/36~.
...
research activity \textemdash i.e. having to do, respectively, with interpretational disagreements
...
alternative approaches to concurrent multi-user document editing
\textemdash RTE and VCS systems.
...
Interface (API) specified by the corpus query engine server application. Resulting query matches
are then merged with the annotations in the local server and transformed into a general
...
controllers for sorting, filtering and paginating \textemdash core functionalities provided by
most corpus query engines.
Hits fetched from the corpus query engine as displayed in the shape of a Key Word In Context (KWIC),
...
ease the synchronization of application state with the backend model \textemdash for instance, the
...
in a research context in which the annotation metadata are created by multiple independent
researchers and have to be synchronized.
We have pointed out 
