...
\begin{abstract}
  ...
  analysis metadata by different reasearchers.
...
not support proper collaborative editing of documents\footnote{
  For instance, DropBox explicitely recommends users to modify shared documents outside the scope
...
rights and enabling more focused annotation queries, projects provide the ground for the
publish-subscribe (pub/sub) notification pattern.
Projects are the target of the branching funtionality described in Section \ref{subsec:dbversion}.
As a result of branching a sibling project is created to which other users can subscribe.
...
Our implementation of an asynchronous collaborative annotation interface is still in developement
and certain aspects are subject to improvement. In particular we would like to highlight the
following two points.

First, while the simplicity of our annotation data model provides an easily maintenable and
implementable abstraction, it does not, however, reflect current advances made by other researchers
with respect to corpus data modelling. For instance, authors related to the ANNIS project have
presented a generic, multi-layer, graph-based data model known as RelANNIS \cite{Krause2016} that
aims at covering an extensive set of annotation types (including subtoken, token, span and
tree annotations). Similarly, members of the CQP developement team have recently released a working
paper describing the data model \textemdash known as Ziggurat \cite{Evert2015} \textemdash to be
used in a future release of the CQP engine. Similarly to RelANNIS, Ziggurat describes a multilayer
...
In the present paper, we have discussed the neccessity of specifically addressing issues arising
...
The first author wants to thank Chris Emmery for useful discussions on previous drafts of this
work. 
