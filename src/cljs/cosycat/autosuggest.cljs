...
(defn startswith [s prefix] (gstr/caseInsensitiveStartsWith s prefix))

(defn find-keys [expr suggs & {:keys [dedup-by]}]
  (->> (if (empty? expr)
         suggs
         (filter-suggs suggs #(startswith (:tag %) expr) :dedup-by dedup-by))
       (mapv (fn [{:keys [name tags]}] {:name name :tags (mapv #(assoc % :val (:tag %)) tags)}))))

(defn find-vals [expr suggs & {:keys [dedup-by]}]
  (if (empty? expr)
    suggs
    (filter-suggs suggs #(startswith (:tag %) expr) :dedup-by dedup-by)))
...
(defn fetch-requested [tag-suggestions suggestions & {:keys [scope]}]
  (fn [value]
    (if-not (has-key value)
      (find-keys value tag-suggestions :dedup-by :tag)
      (let [[key val] (clojure.string/split value #"=")]
        (find-vals val suggestions)))))

(defn wrap-react [my-atom f]
  (fn [arg]
    (reset! my-atom (f (.-value arg)))))

(defn on-change-suggest [value-atom]
  (fn [e new-val] (reset! value-atom (.-newValue new-val))))
...
(defn suggest-annotations
  [tagsets & {:keys [value on-change on-blur type placeholder] :as props}]
  (let [sugg-atom (reagent/atom [])
        value-atom (reagent/atom "")]
    (fn [tagsets & {:keys [value on-change on-blur type placeholder] :as props}]
      (let [suggs (denormalize-tagsets tagsets)
        ...
        [autosuggest
         {:suggestions @sugg-atom
          ...
          :inputProps (assoc props :onChange (on-change-suggest value-atom) :value @value-atom)}]))))
