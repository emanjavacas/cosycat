...
(defn annotation-row [hit hit-num]
  (fn [hit hit-num]
    (into
     [:tr]
     (for [{:keys [id xmlid word anns] :as token} hit
           :let [key (str hit-num "-" id "props")]]
       ^{:key key}
       [:td
        [:table {:style {:font-size "11px" :width "100%" :min-height "50px"}}
         [:thead]
         [:tbody
          (for [{:keys [key value]} example-anns]
            ^{:key (str hit-num "-" id "-anns-" key)}
      ...
      (doall
       (interleave
        (for [[hit-num {:keys [hit meta]}] (sort-by first @marked-hits)]
          ^{:key hit-num} [hit-row hit hit-num])
        (for [[hit-num {:keys [hit meta]}] (sort-by first @marked-hits)]
      ...
      [:div.container
       ...
       (if (zero? (count @marked-hits))
         [error-panel
          ...
          :status-content [bs/button {:href "#/query"}
                           [:span {:style {:padding-right "10px"}}
                            [:i.zmdi.zmdi-long-arrow-left]]
                           "Back to query"]]
