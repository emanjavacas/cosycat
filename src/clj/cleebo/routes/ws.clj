...
(defn connect!
  ...
  (notify-clients {:type :notify
                   :status :ok
                   :data {:message "Hello world!"
                          :by ws-name}} ws-channels ws-ch)
...
(defn disconnect!
  ...
  (let [ws-user (get @ws-channels ws-ch "unknown")]
    ...
    (notify-clients {:type :notify
                     :status :ok
                     :data {:by ws-user
                            :message "Goodbye world!"}} ws-channels ws-ch)
...
(defn on-ws-error [ws-from ws-from-name {:keys [data type] :as payload}]
  (send->client
   :ws-to ws-from
   :payload (assoc payload :status :error)
...
(defn notify-clients [payload ws-channels & [ws-from]]
  (let [ws-from-name (get @ws-channels ws-from "server")]
...
(defn annotation-route
  ...
  (let [{token-id :token-id hit-id :hit-id ann :ann} data
        {username :username {key :key} :ann} ann
        {db :db {ws-channels :ws-channels} :ws} components
        ws-from-name (get @ws-channels ws-from)]
    (if (not= ws-from-name username)
      (throw
       (ex-info "Couldn't verify identity"
                {:token-id token-id :reason :false-id :username username}))
      (try
        (let [{:keys [anns]} (new-token-annotation db (Integer/parseInt token-id) ann)]
          (send->client
           :ws-to ws-from
           :payload {:status :ok
                     :type :annotation
