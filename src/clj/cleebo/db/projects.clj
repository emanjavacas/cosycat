...
(defn process-users [creator users]
  (map #(select-keys % [:username :role])
       (cons {:username creator :role "creator"} users)));add creator to users

(s/defn new-project :- project-schema
  ([db creator] (new-project db creator (default-project-name creator) default-project-desc))
  ([{db-conn :db :as db} creator project-name description & [users]]
   ...
   (-> (mc/insert-and-return db-conn (:projects colls)
                             {:name project-name
                              :creator creator
...
(defn user-projects
  [{db-conn :db} username]
  (->> (mc/find-maps  db-conn (:projects colls)
